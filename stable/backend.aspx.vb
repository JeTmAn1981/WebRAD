...
Partial Public Class backendnew
            ...
            If dt.Rows.Count > 0 Then
                With dt.Rows(0)
                    ...
                    If pnlBackend.Visible = True Then
                        ...
                        CType(ucCustomSelectDataSource, stable.WebRADDataSource).BindData(.Item("CustomSelectDataSourceID"))
    ...
    Private Function ScheduleSelected() As Boolean
        Return BackendOptionSelected("Schedule page")
    End Function

    Function GetOptionTab(tabType As String) As AjaxControlToolkit.TabPanel
        Return (From tab As AjaxControlToolkit.TabPanel In OptionTabs.Tabs Where tab.HeaderText.Replace(" ", "") = tabType Select tab).FirstOrDefault()
    End Function

    Sub LoadExports(ByVal sType As String)
        If GetOptionTab(sType).Visible Then
            ...
            Dim rptCurrent As Repeater = CType(GetPageControlReference("rpt" & sType & "s"), Repeater)
            ...
            For nCounter As Integer = 0 To rptCurrent.Items.Count - 1
                ...
                If sType = "Report" Then
                    SetItemSelected(CType(rptCurrent.Items(nCounter).FindControl("rblTemplateMessageType"), RadioButtonList), dtExports.Rows(nCounter).Item("MessageType"))
                    CType(rptCurrent.Items(nCounter).FindControl("txtTemplate"), TextBox).Text = dtExports.Rows(nCounter).Item("Template")
                    CType(rptCurrent.Items(nCounter).FindControl("txtTemplatePlain"), TextBox).Text = dtExports.Rows(nCounter).Item("Template")
                    rblTemplateMessageType_SelectedIndexChanged(rptCurrent.Items(nCounter).FindControl("rblTemplateMessageType"), Nothing)
                End If
            Next

            SetItemSelected(CType(GetPageControlReference("ddlNumber" & sType & "s"), DropDownList), dtExports.Rows.Count)
        End If
    End Sub
    ...
    Protected Sub cblBackendOptions_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cblBackendOptions.SelectedIndexChanged
        ...
        'Dim projectID = GetProjectID()

        'For Each item As ListItem In cblBackendOptions.Items
        '    Dim currentOption = db.ProjectBackendOptions.FirstOrDefault(Function(bo) bo.ProjectID = projectID And bo.Type = item.Value)

        '    If Not item.Selected AndAlso currentOption IsNot Nothing Then
        '        db.ProjectBackendOptions.Remove(currentOption)
        '        change = True
        '    ElseIf item.Selected AndAlso currentOption Is Nothing Then
        '        db.ProjectBackendOptions.Add(New ProjectBackendOption() With {.Project = currentProject, .ProjectID = projectID, .Type = item.Value})
        '        change = True
        '    End If
        'Next
    ...
    Sub ShowBackendOptions()
        Dim detailOptionsSelected As Integer = 0

        For nCounter As Integer = 0 To cblBackendOptions.Items.Count - 1
            Dim currentTab = (From tab As AjaxControlToolkit.TabPanel In OptionTabs.Tabs
                              Where tab.HeaderText = cblBackendOptions.Items(nCounter).Text
                              Select tab).FirstOrDefault()

            Try
                currentTab.Visible = cblBackendOptions.Items(nCounter).Selected
                detailOptionsSelected += 1
            Catch ex As Exception
            End Try
        Next

        pnlShowOptionDetails.Visible = (detailOptionsSelected > 0)
    End Sub
    ...
    Sub LoadActionOptions()
        Dim dtOptions As DataTable = GetDataTable("Select OptionID, ControlID, Label, OperatorType as ActionType, ComparisonValue as UpdateValue, CustomActionCode from " & DT_WEBRAD_PROJECTBACKENDOPTIONCOLUMNS & "  Where OptionID = (Select ID From " & DT_WEBRAD_PROJECTBACKENDOPTIONS & "  Where Type = 5 and ProjectID = " & Request.QueryString("ID") & ")", _cnx)
    ...
    Protected Sub btnContinue_Click(sender As Object, e As EventArgs) Handles btnContinue.Click
        ...
        If Page.IsValid Then
            ...
            If BackendOptionSelected("Custom select statement") Then
                cmd.Parameters.AddWithValue("@CustomSelectDataSourceID", CType(ucCustomSelectDataSource, stable.WebRADDataSource).SaveData())
            End If
        ...
        For nCounter As Integer = 0 To cblBackendOptions.Items.Count - 1
            If cblBackendOptions.Items(nCounter).Selected Then
                ...
                If cblBackendOptions.Items(nCounter).Text = "Export" Or cblBackendOptions.Items(nCounter).Text = "Report" Then
                    SaveExports(cblBackendOptions.Items(nCounter).Text, CType(GetPageControlReference("rpt" & cblBackendOptions.Items(nCounter).Text & "s"), Repeater))
                End If
            End If
        Next
    ...
    Sub SaveExports(ByVal sType As String, ByVal rptCurrent As Repeater)
        ...
        MakeExportNamesUnique(rptCurrent)
        ...
        For Each currentItem As RepeaterItem In rptCurrent.Items
            With currentItem
                ...
                Else
                    ...
                    cmd.Parameters("@MessageType").Value = CType(.FindControl("rblTemplateMessageType"), RadioButtonList).SelectedValue
                    cmd.Parameters("@Template").Value = If(CType(.FindControl("rblTemplateMessageType"), RadioButtonList).SelectedValue = "Rich", CType(.FindControl("txtTemplate"), TextBox).Text, CType(.FindControl("txtTemplatePlain"), TextBox).Text)
                End If
    ...
    ''' <summary>
    ''' Potential bug:  What happens if the export name is already at the character limit?  Uniqueness-inducing dashes will be truncated?
    ''' </summary>
    ''' <param name="rptCurrent"></param>
    Public Shared Sub MakeExportNamesUnique(ByVal rptCurrent As Repeater)
        Dim names = (From item As RepeaterItem In rptCurrent.Items
                     Select CType(item.FindControl("txtName"), TextBox).Text).Distinct().ToList()

        names.ForEach(Sub(name)
                          Dim namesFound = (From item As RepeaterItem In rptCurrent.Items
                                            Where CType(item.FindControl("txtName"), TextBox).Text = name
                                            Select item).ToList()

                          If namesFound.Count > 1 Then
                              For counter As Integer = 1 To namesFound.Count - 1
                                  CType(namesFound(counter).FindControl("txtName"), TextBox).Text &= New String("_", counter)
                              Next
                          End If
                      End Sub)
    End Sub
    ...
    Sub SaveAncillaryMaintenance()
        ...
        If BackendOptionSelected("Ancillary maintenance") Then
            Dim cmd As New SqlCommand
            cmd.Connection = _cnx
    ...
    Sub SaveActions()
        If BackendOptionSelected("Actions other than delete") Then
            Dim cmd As New SqlCommand
            cmd.CommandType = CommandType.StoredProcedure
            cmd.Connection = _cnx
    ...
    Sub SaveSortInfo()
        If BackendOptionSelected("Sort") Then
            Dim cmd As New SqlCommand
            cmd.CommandType = CommandType.StoredProcedure
            cmd.Connection = _cnx
    ...
    Sub SaveFilterInfo()
        If BackendOptionSelected("Filter") Then
    ...
    Protected Sub ddlActionType_SelectedIndexChanged(sender As Object, e As EventArgs)
        With CType(sender.parent, RepeaterItem)
            If CType(sender, DropDownList).SelectedIndex > 0 Then
                ...
                CType(.FindControl("pnlControl"), Panel).Visible = CType(sender, DropDownList).SelectedValue <> N_BACKENDCUSTOMACTION_TYPE
    ...
    Protected Sub ddlNumberExports_SelectedIndexChanged(sender As Object, e As EventArgs)
        Dim dtSupplied As New WhitTools.DataTablesSupplied
        Dim dtExportColumns As DataTable = GetExportColumns()
        Dim rptCurrent As Repeater = CType(GetPageControlReference("rpt" & Replace(sender.id, "ddlNumber", "")), Repeater)
    ...
    Protected Sub rblTemplateMessageType_SelectedIndexChanged(sender As Object, e As EventArgs)
        With GetParentRepeaterItem(sender)
            UpdateMessageType(CType(sender, RadioButtonList), CType(.FindControl("txtTemplate"), TextBox), CType(.FindControl("txtTemplatePlain"), TextBox))
        End With
    End Sub
End Class
